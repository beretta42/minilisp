...
/* crap needed to get it to compile */
#define byte uint8_t
...
void swap_out_basic_after_print() {
    /*
  asm {
    orcc #$50
    puls d
...
word bread(int fd, char *buf, word numBytesRequested) {
    swap_in_basic();
...
byte bopen(int fd, char *filename) {
    swap_in_basic();
    byte bopen_result = FALSE;
    swap_out_basic();
...
static void forward_root_objects(void *root) {
    ...
    for (frame = (void **)root; frame; frame = *(void ***)frame)
...
void myungetc(char c, byte ignore) {
    if (doing_load) {
	file_data = c;
	has_file_data = TRUE;
	return;
...
static Obj *intern(void *root, const char *name) {
    Obj *p;
    for (p = Symbols; p != Nil; p = p->val.cell.cdr)
...
Obj *myfind(Obj **env, Obj *sym) {
    Obj *p;
    Obj *cell;
    for (p = *env; p != Nil; p = p->val.env.up) {
        for (cell = p->val.env.vars; cell != Nil; cell = cell->val.cell.cdr) {
...
static Obj *macroexpand(void *root, Obj **env, Obj **obj) {
    ...
    *bind = myfind(env, (*obj)->val.cell.car);
...
static Obj *prim_setq(void *root, Obj **env, Obj **list) {
    ...
    DEFINE2(bind, value);
    *bind = myfind(env, (*list)->val.cell.car);
    if (!*bind)
...
static Obj *prim_plus(void *root, Obj **env, Obj **list) {
    long sum = 0;
    Obj *args;
    for (args = eval_list(root, env, list); args != Nil; args = args->val.cell.cdr) {
...
static Obj *prim_minus(void *root, Obj **env, Obj **list) {
    Obj *args = eval_list(root, env, list);
    Obj *p;
    for (p = args; p != Nil; p = p->val.cell.cdr)
    ...
    for (p = args->val.cell.cdr; p != Nil; p = p->val.cell.cdr)
